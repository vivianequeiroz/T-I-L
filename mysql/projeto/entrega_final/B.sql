...
-- SELECIONA data e hora de tarefas cuja descrição contenha a letra `a`
SELECT data, hora FROM tarefa
WHERE descricao LIKE '%a';

-- SELECIONA data e hora das tarefas que estão em andamento
SELECT data, hora FROM tarefa
WHERE usuario_id IN
    (SELECT usuario_id FROM subtarefa_tarefa
     WHERE status = "EM ANDAMENTO");

-- SELECIONA

-- SELECIONA as descrições das tarefas e seus respectivos status ordenando por data
SELECT descricao, status, data FROM tarefa
ORDER BY data;